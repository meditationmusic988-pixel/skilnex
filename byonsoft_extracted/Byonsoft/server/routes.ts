...
Output ONLY this exact JSON structure (no markdown, no text before or after the JSON):
{
  ...
  app.get("/api/admin/drive/import", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      ...
      async function fetchAllFiles(parentId: string, moduleName: string = "", isRoot: boolean = true): Promise<{title: string, video_url: string, module_name: string}[]> {   const query = encodeURIComponent(`'${parentId}' in parents and trashed=false`);   const fields = encodeURIComponent("files(id,name,mimeType)");   const driveRes = await fetch(     `https://www.googleapis.com/drive/v3/files?q=${query}&fields=${fields}&orderBy=folder,name&pageSize=1000&key=${encodeURIComponent(apiKey)}`   );   if (!driveRes.ok) {     const errBody: any = await driveRes.json();     throw new Error(errBody?.error?.message || "Google Drive API error");   }   const driveData: { files: { id: string; name: string; mimeType: string }[] } = await driveRes.json();   const files = driveData.files || [];    const subFolders = files.filter(f => f.mimeType === "application/vnd.google-apps.folder");   const videoFiles = files.filter(f =>     f.mimeType !== "application/vnd.google-apps.folder" && (       f.mimeType.includes("video") ||       f.mimeType === "application/octet-stream" ||       f.name.match(/\.(mp4|mkv|avi|mov|webm|flv)$/i)     )   );    videoFiles.sort((a, b) =>     a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: "base" })   );    subFolders.sort((a, b) =>     a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: "base" })   );    let results: {title: string, video_url: string, module_name: string}[] = [];    // Root folder mein agar directly videos hain (no sub-folders) toh moduleName use karo   if (!isRoot || subFolders.length === 0) {     results = videoFiles.map(f => ({       title: f.name.replace(/\.[^/.]+$/, ""),       video_url: `https://drive.google.com/file/d/${f.id}/preview`,       module_name: moduleName,     }));   }    // Har sub-folder = ek module — folder ka naam module name banega   for (const folder of subFolders) {     const subFiles = await fetchAllFiles(folder.id, folder.name, false);     results = results.concat(subFiles);   }    return results; }
