...
export function AIRoadmapSection({ roadmap, matchedCourses, onImprove, onGetRoadmap }: AIRoadmapSectionProps) {
  if (!roadmap || (!roadmap.career_paths?.length && !roadmap.recommended_courses?.length)) {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Career Paths — EMERALD/GREEN theme */}
        <Card className="p-6 bg-slate-900/50 border-emerald-500/20">
          ...
          <div className="space-y-2">
            {roadmap.career_paths?.map((path: string, i: number) => (
              <div key={i} className="p-2 rounded bg-emerald-500/5 border border-emerald-500/15 text-sm">
                {path}
              </div>
            ))}
          </div>
          {roadmap.expected_income && (
            <div className="mt-4 pt-4 border-t border-emerald-500/10">
              ...
              <div className="text-emerald-400 font-bold">{roadmap.expected_income}</div>
            </div>
          )}
        </Card>

        {/* Recommended Courses — BLUE/CYAN theme */}
        <Card className="p-6 bg-slate-900/50 border-cyan-500/20 md:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {roadmap.recommended_courses?.map((course: any, i: number) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-cyan-500/5 border border-cyan-500/15">
          ...
          {/* Learning Order — array form (from career_analyses roadmap.month1/2/3) */}
          {Array.isArray(roadmap.learning_order) && roadmap.learning_order.length > 0 && (
            <div className="mt-6">
              <div className="flex items-center gap-2 mb-3 text-orange-400">
                <TrendingUp className="w-4 h-4" />
                <span className="text-sm font-bold">Learning Order</span>
              ...
              <div className="space-y-2">
                {roadmap.learning_order.map((step: string, i: number) => (
                  <div key={i} className="text-sm text-gray-300 bg-orange-500/5 p-3 rounded-lg border border-orange-500/10 leading-relaxed">
                    {step}
                  </div>
                ))}
              </div>
